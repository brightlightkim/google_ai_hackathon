...
import { getRoundTrip, getOneWayTrip, getConfig } from "../../../backend/api/skyscannerApi";
const FlightOptions = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const itineraries = await getRoundTrip('Provo', 'Korea', '2024-05-30', '2024-06-01');
  ...
  return (
    <div className='flex-col my-5 border-2 rounded-lg px-4 py-4'>
      ...
      {roundTripItineraries.map((itinerary, index) => (
        <div key={index} className='flex bg-gray-100 justify-center items-center py-2 mb-4 rounded-lg p-2'>
