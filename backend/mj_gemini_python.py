...
travelTo, travelFrom, travelWith, fromDate, toDate, specificActivity = userPreference1

prompt = f"User wants to build a travel plan. You are helpimg the user with provided information. Follow the exmaple prompt for your response. Important: if travel plan is more then 3 days, you have to add additional days according to the travel dates. Example prompt: {example_prompt} \nUse the following information - Destination: {travelTo}, Departure place: {travelFrom}, User traveling with: {travelWith}, From {fromDate} to {toDate}, User wants to do: {specificActivity}\nImportant: Only return a single piece of valid JSON text. Use the same structure of day 1, day 2, and day 3 for additional days. You must include all of the days planned. Your response is going to be used in json.loads(), so check if your response has a proper format to be used in json.loads(). Check if comma is used in the right place. Always use double guotes in your response. Time Zone should be IANA format, and Date should be YYYY.M.D. For example, it should be 2024.8.24 for August 24, 2024. There are Three big sections, (1) Travel_Summary, (2) Daily Planner, and (3) Estimated Budget. Your response should include all of the days planned. For example, if the user is going to travel from July 6 to July 9, you have to provide plans for all of Day 1, Day 2, Day 3, and Day 4."
